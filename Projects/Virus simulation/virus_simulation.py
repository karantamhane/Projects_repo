...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for step in range(300):
            virusPop[step] += float(patient.update())
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        new_resistances = {}
        if res and random.random() <= self.getMaxBirthProb()*(1-popDensity):
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    if numViruses < maxPop:
        viruses = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb) for v in range(numViruses)]
    else:
        viruses = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb) for v in range(1000)]
    ...
    for trial in range(numTrials):
        ...
        for step in range(300):
            ...
            virusPop[step] += float(patient.update())
            resistPop[step] += float(patient.getResistPop(['guttagonol']))
    for pop in virusPop:
        pop /= float(numTrials)
    for rpop in resistPop:
        rpop /= float(numTrials)

    pylab.plot(range(300), virusPop, label = 'Total pop')
    pylab.plot(range(300), resistPop, label = 'Resistant pop')
    pylab.title('Total and resistant virus pop')
    pylab.xlabel('Time steps')
    pylab.ylabel('Virus population')
    pylab.legend(loc = 'best')
    pylab.show()
